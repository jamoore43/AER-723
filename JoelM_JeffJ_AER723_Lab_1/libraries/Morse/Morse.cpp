...
char MorseCode::buttonPress() {
  ...
  if ((holdTime) > 0 && (holdTime) <= 200) {
    DatByte_in = '.';
  }
  else if ((holdTime) > 200 && (holdTime) < 2000) {
    DatByte_in = '-';
  }
...
void MorseCode::translate(String morseString, int counter) {
  ...
  while (1) {
    ...
    letterMorse(letter); // Send Morse Code to letterMorse to obtain the equivalent engish character
